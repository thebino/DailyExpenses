...
class DailyExpensesTests {
    ...
    fun `get expenses with authentication should succeed`() = testApplication {
        ...
        // then
        Assert.assertEquals(HttpStatusCode.OK, response.status)
        Assert.assertEquals(0, response.body<List<NetworkExpense>>().size)
    }
    ...
    fun `get expenses with auth should return all expenses`() = testApplication {
        ...
        // then
        Assert.assertEquals(HttpStatusCode.OK, response.status)
        Assert.assertEquals(2, response.body<List<NetworkExpense>>().size)
    }
